...
class WalletMiniscriptDecayingMultisigDescriptorPSBTTest(BitcoinTestFramework):
    ...
    def _get_xpub(wallet, internal):
        """Extract the wallet's xpubs using `listdescriptors` and pick one."""
        descriptor = next(filter(lambda d: d["desc"].startswith("tr(") and d["internal"] == internal, wallet.listdescriptors()["descriptors"]))
        ...
        return descriptor["desc"].split("tr(")[1].split(")")[0]
    ...
    def run_test(self):
        ...
        deposit_amount = 6.15
        coordinator_wallet.sendtoaddress(multisig.getnewaddress(address_type="bech32"), deposit_amount)
