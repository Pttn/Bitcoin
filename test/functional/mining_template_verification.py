...
class MiningTemplateVerificationTest(BitcoinTestFramework):
    ...
    def nbits_test(self, node, block):
        ...
        bad_block = copy.deepcopy(block)
        bad_block.nBits = 4294967295  # impossible in the real world
        assert_template(node, bad_block, "bad-diffbits", solve=False, expect_submit="short-constellation")

        self.log.info("Altering nBits should make the block invalid")
        bad_block = copy.deepcopy(block)
        bad_block.nBits += 1
