...
class AssumeValidTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=[
            f"Enabling script and PoW verification at block #1 ({block_1_hash}): assumevalid=0 (always verify).",
        ...
        with self.nodes[1].assert_debug_log(expected_msgs=[
            f"Disabling script and PoW verification at block #1 ({self.blocks[0].hash_hex}).",
            f"Enabling script and PoW verification at block #103 ({self.blocks[102].hash_hex}): block height above assumevalid height.",
        ]):
        ...
        with self.nodes[2].assert_debug_log(expected_msgs=[
            f"Enabling script and PoW verification at block #1 ({block_1_hash}): block too recent relative to best header.",
            "Block validation error: block-script-verify-flag-failed",
        ]):
        ...
        with self.nodes[3].assert_debug_log(expected_msgs=[
            f"Enabling script and PoW verification at block #1 ({block_1_hash}): block not in best header chain.",
        ]):
        ...
        with self.nodes[4].assert_debug_log(expected_msgs=[
            f"Enabling script and PoW verification at block #1 ({alt1.hash_hex}): block not in assumevalid chain.",
        ]):
        ...
        with self.nodes[5].assert_debug_log(expected_msgs=[
            f"Enabling script and PoW verification at block #1 ({block_1_hash}): assumevalid hash not in headers.",
        ]):
        ...
        with self.nodes[5].assert_debug_log(expected_msgs=[
            f"Enabling script and PoW verification at block #1 ({block_1_hash}): best header chainwork below minimumchainwork.",
        ]):
