...
class WalletMultisigDescriptorPSBTTest(BitcoinTestFramework):
    ...
    def _get_xpub(wallet):
        ...
        pkh_descriptor = next(filter(lambda d: d["desc"].startswith("wpkh(") and not d["internal"], wallet.listdescriptors()["descriptors"]))
        ...
        return pkh_descriptor["desc"].split("wpkh(")[1].split(")")[0].replace("/0/*", "/<0;1>/*")
