...
std::shared_ptr<CWallet> SetupDescriptorsWallet(interfaces::Node& node, TestChain100Setup& test, bool watch_only = false)
{
    ...
    CTxDestination dest = WitnessV1Taproot(XOnlyPubKey(test.coinbaseKey.GetPubKey()));
...
void TestGUIWatchOnly(interfaces::Node& node, TestChain100Setup& test)
{
    ...
    // Set change address
    sendCoinsDialog.getCoinControl()->destChange = WitnessV1Taproot(XOnlyPubKey(test.coinbaseKey.GetPubKey()));
