...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, int64_t spendheight, bool enforceMinFee) const
{
    ...
    for (const auto& it : score_with_topo) {
        ...
        assert(Consensus::CheckTxInputs(tx, dummy_state, mempoolDuplicate, spendheight, txfee, enforceMinFee));
