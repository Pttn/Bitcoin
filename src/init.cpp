 ...
 */
static const char* BITCOIN_PID_FILENAME = "riecoind.pid";
/**
...
#endif
    argsman.AddArg("-assumevalid", "Skip some checks on ancestors of a recent hard coded Block, avoiding a very long first Sync. -noassumevalid to disable.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogWarning("Relative datadir option '%s' specified, which will be interpreted relative to the "
                   "current working directory '%s'. This is fragile, because if Riecoin is started in the future "
                   "from a different location, it will be unable to locate the current data files. There could "
                   "also be data loss if Riecoin is started while in a temporary directory.",
