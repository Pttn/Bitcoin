...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " riecoin-tx utility version " + FormatFullVersion() + "\n";
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                ...
                "\n"
                "Usage: riecoin-tx [options] <hex-tx> [commands]\n"
                "or:    riecoin-tx [options] -create [commands]\n"
                "\n";
