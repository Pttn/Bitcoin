...
public:
    ...
    {
        return btck_block_tree_entry_get_time(get());
    }
...
public:
    ...
    virtual void HeaderTipHandler(SynchronizationState state, BlockTreeEntry entry) {}
    ...
    void SetNotifications(std::shared_ptr<T> notifications)
    {
        ...
        btck_context_options_set_notifications(
            ...
            btck_NotificationInterfaceCallbacks{
                ...
                .header_tip = +[](void* user_data, btck_SynchronizationState state, const btck_BlockTreeEntry* entry) { (*static_cast<user_type>(user_data))->HeaderTipHandler(static_cast<SynchronizationState>(state), BlockTreeEntry{entry}); },
