...
public:
    ...
    {
        if (m_cbs.header_tip) m_cbs.header_tip(m_cbs.user_data, cast_state(state), btck_BlockTreeEntry::ref(i));
    }
...
int64_t btck_block_tree_entry_get_time(const btck_BlockTreeEntry* entry)
{
    return btck_BlockTreeEntry::get(entry).nTime;
}
...
{
    bool new_block;
    auto result = btck_ChainstateManager::get(chainman).m_chainman->ProcessNewBlock(btck_Block::get(block), /*force_processing=*/true, /*new_block=*/&new_block);
    if (_new_block) {
    ...
    btck_BlockValidationState* state)
{
    try {
        auto& chainman = btck_ChainstateManager::get(chainstate_manager).m_chainman;
        auto result = chainman->ProcessNewBlockHeaders({&btck_BlockHeader::get(header), 1}, btck_BlockValidationState::get(state), /*ppindex=*/nullptr);
...
uint8_t* btck_block_header_get_nonce(const btck_BlockHeader* header)
{
    uint8_t *u256((uint8_t*) malloc(32));
    for (uint16_t i(0); i < 32; i++)
        u256[i] = ArithToUint256(btck_BlockHeader::get(header).nNonce).data()[i];
    return u256;
}
