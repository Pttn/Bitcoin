                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (auto& queue = m_chainman.GetCheckQueue(); queue.HasThreads() && fScriptPoWChecks) control.emplace(queue);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase() && fScriptPoWChecks)
        {
...
bool ChainstateManager::NotifyHeaderTip()
{
    ...
    if (fNotify) {
        GetNotifications().headerTip(GetSynchronizationState(fInitialBlockDownload, m_blockman.m_blockfiles_indexed), pindexHeader);
    }
    return fNotify;
}
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nTime <= 1706630867) // Latest Block larger than current Weight Limit, 2082094
...
void ChainstateManager::UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev) const
{
    ...
    if (commitpos != NO_WITNESS_COMMITMENT && !block.vtx[0]->HasWitness()) {
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // Enforce BIP113 (Median Time Past) (except for Genesis).
    ...
    // Enforce rule that the coinbase starts with serialized block height (except for Genesis).
    if (nHeight != 0)
    {
    ...
    if (!CheckWitnessMalleation(block, true, state)) {
        return false;
    }
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool checkPoW)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
...
bool Chainstate::NeedsRedownload() const
{
    ...
    // Segwit consensus rules must be validated
    CBlockIndex* block{m_chain.Tip()};

    while (block != nullptr && (m_chainman.GetParams().GetChainType() != ChainType::MAIN || block->nHeight >= 1096704)) {
